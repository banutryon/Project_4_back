    ...
    def self.all
        ...
        def self.update(id, new)
            results = DB.exec(
                <<-SQL
                UPDATE locations
                SET name='#{new["name"]}', street='#{new["street"]}', city='#{new["city"]}', state='#{new["state"]}', zipcode='#{new["zipcode"]}', img1='#{new["img1"]}', img2='#{new["img2"]}', img3='#{new["img3"]}', description='#{new["description"]}'
                WHERE id=#{id}
                RETURNING id, name, street, city, state, zipcode, img1, img2, img3, description;
            SQL
        )
